...
/**
 * Utility for working with float arrays and primitive collections.
...
public class FloatArrayUtil {
     ...
     */
    public static void sort(final float[] array, final FloatComparator comparator) {
     ...
     * @throws NullPointerException if any arguments are null
     */
    public static void sort(final float[] array, final int lower, final int upper, final FloatComparator comparator) {
        quickSort(array, lower, upper, comparator, new Random());
    }

    private static void insertSort(final float[] array, final int lower, final int upper, final FloatComparator comparator) {
    ...
    private static void quickSort(final float[] array, final int lower, final int upper, final FloatComparator comparator, final Random random) {
        if (upper - lower < MIN_QSORT_SIZE) {
            insertSort(array, lower, upper, comparator);
            return;
        }
        swap(array, lower, lower + random.nextInt(upper - lower + 1));
        final float t = array[lower];
        int i = lower;
        int j = upper + 1;
        while (true) {
            do {
    ...
    /**
     * Interface for comparing two floats.
     */
