...
/**
 * Utility for working with short arrays and primitive collections.
...
public class ShortArrayUtil {
     ...
     */
    public static void sort(final short[] array, final ShortComparator comparator) {
     ...
     * @throws NullPointerException if any arguments are null
     */
    public static void sort(final short[] array, final int lower, final int upper, final ShortComparator comparator) {
        quickSort(array, lower, upper, comparator, new Random());
    }

    private static void insertSort(final short[] array, final int lower, final int upper, final ShortComparator comparator) {
    ...
    private static void quickSort(final short[] array, final int lower, final int upper, final ShortComparator comparator, final Random random) {
        if (upper - lower < MIN_QSORT_SIZE) {
            insertSort(array, lower, upper, comparator);
            return;
        }
        swap(array, lower, lower + random.nextInt(upper - lower + 1));
        final short t = array[lower];
        int i = lower;
        int j = upper + 1;
        while (true) {
            do {
    ...
    /**
     * Interface for comparing two shorts.
     */
