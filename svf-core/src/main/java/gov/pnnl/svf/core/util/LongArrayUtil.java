...
public class LongArrayUtil {
     ...
     */
    public static void sort(final long[] array, final LongComparator comparator) {
     ...
     * @throws NullPointerException if any arguments are null
     */
    public static void sort(final long[] array, final int lower, final int upper, final LongComparator comparator) {
        quickSort(array, lower, upper, comparator, new Random());
    }

    private static void insertSort(final long[] array, final int lower, final int upper, final LongComparator comparator) {
    ...
    private static void quickSort(final long[] array, final int lower, final int upper, final LongComparator comparator, final Random random) {
        if (upper - lower < MIN_QSORT_SIZE) {
            insertSort(array, lower, upper, comparator);
            return;
        }
        swap(array, lower, lower + random.nextInt(upper - lower + 1));
        final long t = array[lower];
        int i = lower;
        int j = upper + 1;
        while (true) {
            do {
    ...
    /**
     * Interface for comparing two longs.
     */
