...
/**
 * Utility for working with byte arrays and primitive collections.
...
public class ByteArrayUtil {
     ...
     */
    public static void sort(final byte[] array, final ByteComparator comparator) {
     ...
     * @throws NullPointerException if any arguments are null
     */
    public static void sort(final byte[] array, final int lower, final int upper, final ByteComparator comparator) {
        quickSort(array, lower, upper, comparator, new Random());
    }

    private static void insertSort(final byte[] array, final int lower, final int upper, final ByteComparator comparator) {
    ...
    private static void quickSort(final byte[] array, final int lower, final int upper, final ByteComparator comparator, final Random random) {
        if (upper - lower < MIN_QSORT_SIZE) {
            insertSort(array, lower, upper, comparator);
            return;
        }
        swap(array, lower, lower + random.nextInt(upper - lower + 1));
        final byte t = array[lower];
        int i = lower;
        int j = upper + 1;
        while (true) {
            do {
    ...
    /**
     * Interface for comparing two bytes.
     */
