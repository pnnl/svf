...
/**
 * Utility for working with int arrays and primitive collections. Implementation
 * based on qsort4 from Programming Pearls.
 *
...
public class IntArrayUtil {
     ...
     */
    public static void sort(final int[] array, final IntComparator comparator) {
     ...
     * @throws NullPointerException if any arguments are null
     */
    public static void sort(final int[] array, final int lower, final int upper, final IntComparator comparator) {
        quickSort(array, lower, upper, comparator, new Random());
    }

    private static void insertSort(final int[] array, final int lower, final int upper, final IntComparator comparator) {
    ...
    private static void quickSort(final int[] array, final int lower, final int upper, final IntComparator comparator, final Random random) {
        if (upper - lower < MIN_QSORT_SIZE) {
            insertSort(array, lower, upper, comparator);
            return;
        }
        swap(array, lower, lower + random.nextInt(upper - lower + 1));
        final int t = array[lower];
        int i = lower;
        int j = upper + 1;
        while (true) {
            do {
    ...
    /**
     * Interface for comparing two ints.
     */
